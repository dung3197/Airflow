...
# New requirement:
# Generate about 100 Avro files, each about 128 MB
FILE_COUNT = 100
...
# Batch size per expansion round while estimating target size
BATCH_SIZE = 1000

# File naming pattern
FILE_NAME_TEMPLATE = "ogg_customer_events_{file_index:04d}.avro"
...
def generate_customer_id(sequence_number: int) -> int:
    ...
    This guarantees uniqueness within one full multi-file generation run.
    """
...
def build_customer_row(customer_id: int, full_name: str) -> Dict[str, Any]:
    ...
    Important:
    - customer_id is unique within the whole generation run
...
def generate_single_avro_file(
    ...
    """
    Generate one Avro file until its size reaches at least target_size_bytes.

    Returns:
        total_records_in_file,
        next_sequence_number_to_use
    """
    records: List[Dict[str, Any]] = []
    next_sequence_number = start_sequence_number
    ...
    while True:
        ...
        write_avro_file(output_file, records)

        current_size = get_file_size(output_file)
        print(
            f"[{output_file.name}] size={current_size / (1024 * 1024):.2f} MB | "
    ...
    logger.info(
        "Avro file generated | file=%s | size_bytes=%d | size_mb=%.2f | records=%d | customer_id_start=%d | customer_id_end=%d",
    ...
    return len(records), next_sequence_number


def generate_multiple_avro_files(
    output_dir: Path,
    file_count: int,
    target_size_bytes: int,
    batch_size: int,
    ...
    """
    Generate multiple Avro files.
    customer_id remains unique across the entire run.
    """
    output_dir.mkdir(parents=True, exist_ok=True)

    next_sequence_number = 1
    total_records_all_files = 0
    total_size_all_files = 0

    for file_index in range(1, file_count + 1):
        file_name = FILE_NAME_TEMPLATE.format(file_index=file_index)
        output_file = output_dir / file_name

        print(f"\nGenerating file {file_index}/{file_count}: {output_file.name}")

        records_in_file, next_sequence_number = generate_single_avro_file(
        ...
        total_records_all_files += records_in_file
        total_size_all_files += file_size

        print(
            f"Completed {output_file.name} | "
            f"final_size={file_size / (1024 * 1024):.2f} MB | "
            f"records={records_in_file}"
        )

    logger.info(
        "Multi-file generation completed | file_count=%d | total_size_bytes=%d | total_size_mb=%.2f | total_records=%d | last_customer_id=%d",
        file_count,
        total_size_all_files,
        total_size_all_files / (1024 * 1024),
        total_records_all_files,
        next_sequence_number - 1,
    )
    ...
    print(f"Target per file    : {target_size_bytes / (1024 * 1024):.2f} MB")
    print(f"Total size         : {total_size_all_files / (1024 * 1024):.2f} MB")
    print(f"Total records      : {total_records_all_files}")
    print(f"Last customer_id   : {next_sequence_number - 1}")
