...
# Function to check if customer_id or account_number exists
def check_existing_ids(cursor, customer_id, account_number):
    cursor.execute("SELECT COUNT(*) FROM customer_information WHERE customer_id = :1", (customer_id,))
    id_count = cursor.fetchone()[0]
    cursor.execute("SELECT COUNT(*) FROM customer_information WHERE account_number = :1", (account_number,))
    acc_count = cursor.fetchone()[0]
    return id_count > 0 or acc_count > 0

# Function to generate and insert data in batches
def generate_and_insert_data(total_records=1_000_000, batch_size=1_000):
    ...
    try:
        ...
        with open(log_file, 'w') as f:
            ...
            while records_processed < total_records:
                batch = []
                for _ in range(min(batch_size, total_records - records_processed)):
                    record = generate_customer()
                    customer_id, account_number = record[0], record[1]
                    
                    # Check if customer_id or account_number already exists
                    while check_existing_ids(cursor, customer_id, account_number):
                        f.write(f"Skipping duplicate: customer_id={customer_id}, account_number={account_number}\n")
                        print(f"Skipping duplicate: customer_id={customer_id}, account_number={account_number}")
                        skipped_records += 1
                        record = generate_customer()
                        customer_id, account_number = record[0], record[1]
                    ...
                    if error.code == 1:  # ORA-00001: unique constraint violated
                        ...
                        for record in batch:
                            try:
                                ...
                                error2, = e2.args
                                if error2.code == 1:
                                    f.write(f"Skipped record due to unique constraint: customer_id={record[0]}, account_number={record[1]}\n")
                                    print(f"Skipped record due to unique constraint: customer_id={record[0]}, account_number={record[1]}")
                                    skipped_records += 1
                                else:
            ...
            print(f"Data insertion completed successfully. Total records inserted: {records_processed:,}, Skipped: {skipped_records}")
