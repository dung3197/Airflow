...
# Table creation SQL with NOLOGGING for faster initial load
create_table_sql = """
...
) TABLESPACE your_tablespace NOLOGGING
"""

# Function to generate a single row of synthetic data with total_amount calculated
def generate_transaction_row(transaction_id, base_date):
    ...
    return (
        ...
        random.choice(['Yes', 'No', 'N/A']),  # regulatory_flag
...
def process_batch(args):
    start_id, batch_size, base_date = args
    rows = []
    for i in range(batch_size):
        transaction_id = start_id + i
        rows.append(generate_transaction_row(transaction_id, base_date))
    return rows

# Main function with parallel processing and connection pooling
def main():
    try:
        ...
        cursor.prefetchrows = 100001

        # Parallel data generation and insertion
        batch_size = 50000
        ...
        logging.info(f"Inserting {total_records} records in {num_batches} batches with {num_processes} processes...")
        start_time = time.time()

        with Pool(processes=num_processes) as pool:
            batch_args = [(i * batch_size, batch_size, base_date) for i in range(num_batches)]
            results = pool.map(process_batch, batch_args)

            # Bulk insert all batches
            for batch_num, rows in enumerate(results):
                cursor.executemany(
        ...
        if remaining_records > 0:
            rows = [generate_transaction_row(num_batches * batch_size + i + 1, base_date) for i in range(remaining_records)]
            cursor.executemany(
                """INSERT /*+ APPEND */ INTO stock_transactions (
                    transaction_id, trade_timestamp, trader_id, trader_name, trader_email,
                    trader_department, stock_symbol, stock_name, exchange_code, market_type,
        ...
        # Switch back to LOGGING and create indexes
        cursor.execute("ALTER TABLE stock_transactions NOLOGGING OFF")
